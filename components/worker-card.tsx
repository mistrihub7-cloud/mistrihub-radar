  ...
  const statusClass =
    worker.status === "Available Today"
      ...
      : worker.status === "Busy"
        ...
        : "status-offline";
  return (
    <article className={`card flex flex-col p-4 transition hover:-translate-y-0.5 hover:shadow-card ${compact ? "" : "h-full"}`}>
      <div className="flex items-start gap-3">
        {worker.profilePhoto ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img
            alt={worker.name}
            className="h-20 w-20 shrink-0 rounded-2xl border border-slate-200 object-cover object-top shadow-sm ring-4 ring-blue-50"
            src={worker.profilePhoto}
          />
        ) : (
          <ProfessionalAvatar className="h-20 w-20 shrink-0" name={worker.name} />
        )}
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 items-start justify-between gap-2">
            ...
            <span className={`status-pill shrink-0 whitespace-nowrap px-3 py-1 text-xs ${statusClass}`}>{worker.status}</span>
          </div>
          <p className="mt-1 text-sm leading-5 text-slate-500">Serving your area</p>
      ...
      {!compact && (
        <>
          <div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs text-slate-600">
            <span className="grid min-w-0 content-center rounded-xl bg-slate-50 px-2 py-2">
            ...
            <span className="grid content-center rounded-xl bg-slate-50 px-2 py-2">
